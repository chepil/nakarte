...
function getRadioTileCoords(coords, viewTileSize) {
    let z = coords.z - 2;
    if (z < 0) {
        z = 0;
    }
...
async function parseTile(s, projectObj, requestedCoords) {
    ...
    for (let line of lines.slice(2)) {
        ...
        if (fields[6] !== '1') {
            throw new Error(`Unknown radio polygon encoding type: "${fields[6]}"`);
        }
...
export {getTileId, getRadioTileCoords, parseTile, makeTilePath};
